...
class Exchanger(threading.Thread):
    ...
    def update_pl(self):
        try:
            jsonresp = self.get_json('api.prelude.io', "/last/VTC")
        except Exception:
            return
        try:
            jsonresp_usd = self.get_json('api.prelude.io', "/last-usd/VTC")
        except Exception:
            return
    ...
    def update_pb(self):
        quote_currencies = {}
        try:
            jsonresp = self.get_json('api.payb.ee', "/rates/crypto.VTC")
        except Exception:
            return
        for cur in jsonresp:
            try:
