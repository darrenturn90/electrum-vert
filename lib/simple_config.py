...
"""
    ...
    def read_system_config(self):
        """Parse and store the system config settings in electrum-vert.conf into system_config[]."""
        name = '/etc/electrum-vert.conf'
        if os.path.exists(name):
            try:
                import ConfigParser
            except ImportError:
                print "cannot parse electrum-vert.conf. please install ConfigParser"
                return
