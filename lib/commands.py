...
payto_syntax = "payto <recipient> <amount> [label]\n<recipient> can be a vertcoin address or a label"
...
register_command('createmultisig',       2, 2, False, True,  False, 'similar to vertcoind\'s command')
register_command('createrawtransaction', 2, 2, False, True,  False, 'similar to vertcoind\'s command')
...
register_command('getpubkeys',           1, 1, False, True,  False, 'Return the public keys for a wallet address', 'getpubkeys <vertcoin address>')
...
register_command('signrawtransaction',   1, 3, False, True,  True,  'similar to vertcoind\'s command')
    ...
    def _mktx(self, outputs, fee = None, change_addr = None, domain = None):
        ...
        for to_address, amount in outputs:
            if not is_valid(to_address):
                raise Exception("Invalid Vertcoin address", to_address)

        if change_addr:
            if not is_valid(change_addr):
                raise Exception("Invalid Vertcoin address", change_addr)

        if domain is not None:
            for addr in domain:
                if not is_valid(addr):
                    raise Exception("invalid Vertcoin address", addr)
